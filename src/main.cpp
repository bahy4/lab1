...
void print_histogram(Rollable& r, const std::string& name, unsigned minn, unsigned maxx, unsigned width, unsigned NUM=10000){
    ...
    std::cout<<"Математическое ожидание = "<< expected_value(r,NUM)<<std::endl;
    for (unsigned i = minn; i<=maxx; ++i){
...
int main() {
    ...
    DoubleDice doub100 (std::make_unique<Dice>(100,10));
    ...
    print_histogram(doub100, "Бросок d100 с приемуществом + с помехой", 2, 200, 60,NUM);
    return 0;
} 